...
def connector_base_cursor():  # Makes connection to DataBase "Bot"
    conn = sqlite3.connect("Bot.db", check_same_thread=False)
    cursor = conn.cursor()
    return conn, cursor


def execute(filter):  # Makes request to DataBase and returns values (list) using special 'filter'
...
def table_operate(filter):  # Makes request to DataBase and changes it in a proper way using 'filter
    conn, cursor = connector_base_cursor()
...
def exist_test_insert(filter, action1,
                      action2):  # Allows us to avoid problems with DataBase with fields that are UNIQUE
